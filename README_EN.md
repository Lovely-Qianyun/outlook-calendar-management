...
A toolkit that lets an AI assistant (agent) manage your Outlook calendar (Microsoft account / outlook.com) through conversation. Just say "what times are free on Friday afternoon?" or "move the weekly sync to Wednesday" - the AI does the rest: view, add, modify, move, and delete events, including recurring events (e.g. a meeting that repeats every Monday) and free-time lookup. No need to open the Outlook app - your calendar stays in sync across phone, computer, and web.

## Who Is This For?

This project is positioned as a **skill for agents**: `SKILL.md` is the operation manual agents read; this README is the project homepage for humans.

New to this kind of project? Here are the key terms:

- **Agent**: an AI assistant that can call tools to get things done, e.g. Claude Code, Hermes, and similar coding assistants
- **Skill**: a "manual + toolkit" for agents - essentially a directory that says "what this tool can do, how to use it, and when to use it". Once an agent reads it, it knows how to use the tool
- **Recurring event**: an event that repeats automatically on a rule, e.g. a weekly sync every Monday at 9:00 - create it once, it keeps recurring
- **Output protocol**: the fixed format contract of the program's output (see "How It Works"), so agents and scripts never misparse results
...
python scripts/outlook_cal.py add "Weekly sync" "2026-08-10 09:00" "2026-08-10 10:00" --remind 10
```
...
📅 Mon, Aug 10
    🕐 08/10 09:00 - 08/10 10:00  Weekly sync 🔁Weekly on Monday [Work]
    🆔 AAMkAD...
...
🗑️ Removed this occurrence "Weekly sync" (other occurrences kept)
```
...
├── SKILL.md            # Agent operation manual (when to trigger + how to use)
...
Want to contribute? Start with `DEVELOPMENT.md` - it covers the full output protocol, key design decisions, i18n (internationalization) conventions, and official Graph API documentation links.
...
- **Keep zh output word-for-word stable**: the copy is pinned by tests verbatim; changes require syncing the test assertions and bumping the version number - don't change it casually
